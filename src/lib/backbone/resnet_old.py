...
class BBN_ResNet(nn.Module):
    ...
    def load_model(self, pretrain):
        print("Loading Backbone_ pretrain model from {}......".format(pretrain))
        model_dict = self.state_dict()
        pretrain_dict = torch.load(pretrain)
        pretrain_dict = pretrain_dict["state_dict"] if "state_dict" in pretrain_dict else pretrain_dict
        from collections import OrderedDict
...
def bbn_res50(
    cfg,
    pretrain=True,
    pretrained_model="C:/Users/user/Downloads/iNat_2018_InceptionV3.pth",
    last_layer_stride=2,
):
    resnet = BBN_ResNet(
        cfg,
    ...
    if pretrain and pretrained_model != "":
        print("pretrained_model_", pretrained_model)
        resnet.load_model(pretrain=pretrained_model)
    else:
        print("Choose to train from scratch")
        print("pretrained_model_", pretrained_model)
